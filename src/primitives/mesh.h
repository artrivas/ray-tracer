...
public:
    ...
    bool hit(const ray &r, interval ray_t, hit_record &rec) const override {
        ...
        for (int i = 0; i < this->shapes[0].mesh.indices.size() / 3; i++) {
            ...
            const int &idx2 = shapes[0].mesh.indices.at(3 * i + 2).vertex_index;

            const point3 vtx1{attrib.vertices.at(3 * idx0), attrib.vertices.at(3 * idx0 + 1),
                              attrib.vertices.at(3 * idx0 + 2)};
            const point3 vtx2{attrib.vertices.at(3 * idx1), attrib.vertices.at(3 * idx1 + 1),
                              attrib.vertices.at(3 * idx1 + 2)};
            const point3 vtx3{attrib.vertices.at(3 * idx2), attrib.vertices.at(3 * idx2 + 1),
